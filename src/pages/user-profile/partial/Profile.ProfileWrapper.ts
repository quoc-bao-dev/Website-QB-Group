...
class ProfielWarpper extends QBComponent<ProfileWrapperProps, ProfileWrapperState> {
                        ...
                        <div class="size-[80px] rounded-full overflow-hidden relative btn-change-avatar">
                            ${
                                userReducer.getData?.image
                                    ? `<img src="${userReducer.getData?.image}" class="w-full aspect-[1/1] object-cover z-0" alt="">`
                        ...
                        <div class="pt-5">
                            ...
                            <div class="flex gap-2 items-center">
                                <div
                                    ...
                                    <p class="to-gray-500 user-email-text">${userReducer.getData?.email}</p>
                                </div>
                                <div
                            ...
                            <div class="flex flex-col gap-3 pt-3 pr-5">
                                    <div class=" w-full font-semibold text-gray-700 flex items-center gap-2 ">
                                        <p class="">User name: </p>
                                        <p class="">${userReducer.getData?.username}</p>
                                    </div>
    ...
    protected addEventListener(): void {
        this.signEvent('.btn-copy', 'click', () => {
            const text = (this.node('.user-email-text') as HTMLInputElement).innerHTML;
    ...
    protected async afterRender(): Promise<void> {
        const totalPayment = await orderService.getToTalPaymentByUserId(userReducer.getData?.userId!);
        if (totalPayment) {
    ...
    private async changeAvatar() {
        ...
        if (avatar) {
            const res = await userService.updateUserById(userReducer.getData?.userId || '', { avatar: avatar });
            if (res) {
