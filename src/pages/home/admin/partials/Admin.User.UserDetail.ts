...
class UserDetail extends QBComponent<IUser, UserDetailState> {
        ...
        switch (this.state.pageContent) {
            case 'user-history':
                this.renderComponent(
                    ...
                    new AdminUserOrder({ userId: this.props._id })
                );
                break;
            ...
            case 'user-address':
                this.renderComponent(
                    '#admin-user-page-content',
                    new AdminUserAddress({ userId: this.props._id })
                );
                break;
            ...
            case 'user-comment':
                this.renderComponent(
                    '#admin-user-page-content',
                    new AdminUserComments({ userId: this.props._id })
                );
                break;
    ...
    protected addEventListener(): void {
        this.signEventAll('.tab-item', 'click', (e) => {
            ...
            const pageContent = (tab.closest('.tab-item') as HTMLElement)
                ?.dataset.tab;
            this.setState({ pageContent });
            window.history.replaceState(
                {},
                '',
                '/admin/?page=user&user_id=' +
                    this.props._id +
                    '&tab=' +
                    pageContent
            );
    ...
    protected async afterRender(): Promise<void> {
        const order = (await orderService.getOrderByUserId(
            this.props._id
        )) as Order[];
        if (order) {
